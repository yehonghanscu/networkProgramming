...
public class ClientRegister {
    public static void main(String[] args) {
        System.out.println("------------注册客户端启动-------------"+ ServerThread.SDF.format(new Date()));
        byte[] registerRequestData;
        ...
        try (
            ...
            System.out.println("请输入您想注册的用户名（由数字和字母及_组成）:");
            ...
            if(checKRegex(name)){
                ...
                if(checKRegex(password)){
                    //封装数据包并发送到服务端
                    registerRequestData= DataPack.getRequestData(1, name , password);
                    printStream.write(registerRequestData);
                    //处理服务端响应数据
